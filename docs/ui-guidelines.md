...
- カレンダー予定: 色付きカード + タイムライン (Journal と同じ `TimelineItem`)。イベントはタップで編集シート。
...
- 予定追加/変更: Today / Journal から開ける `CalendarEventEditorView` で完結。  
- ダブルタップ追加の説明文を Calendar / Journal の上下に配置。

## ナビゲーション & 操作メモ
- Today や Journal のタイムラインに「ダブルタップで追加」注記を設置。
- Journal 週ビューで別日を開いた際は「今日へ戻る」ボタン。今日を表示中は非表示。
